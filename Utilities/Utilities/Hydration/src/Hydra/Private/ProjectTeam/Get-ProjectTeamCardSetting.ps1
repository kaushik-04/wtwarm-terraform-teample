<#
.SYNOPSIS
Get the current card settings for a given backlog level

.DESCRIPTION
Get the current card settings for a given backlog level, e.g. rows & columns
...
.EXAMPLE
Get-ProjectTeamCardSetting -organization 'contoso' -project 'Module Playground' -team 'product-council' -backloglevel 'Product Portfolio'

Fetch the details for the backlog level 'Product Portfolio' of team 'product-council' of project [contoso|Module Playground]
#>
function Get-ProjectTeamCardSetting {
    ...
    $restInfo = Get-RelativeConfigData -configToken 'RESTeamCardRuleSettingsGet'
    $restInputObject = @{
