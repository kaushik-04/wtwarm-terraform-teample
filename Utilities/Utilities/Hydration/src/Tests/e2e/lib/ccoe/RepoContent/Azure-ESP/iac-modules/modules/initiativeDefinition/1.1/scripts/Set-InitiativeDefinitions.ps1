...
param (
    ...
    [string]$InitiativeDefinitionSchemaFilePath = "$PSScriptRoot/initiativeDefinitionSchema.json",
    ...
    if ($DeleteIfMarkedForDeletion.IsPresent) {
        ...
        foreach ($item in $definitionSetsToDelete) {
            ...
            if ($PSCmdlet.ShouldProcess(("PolicySet definition [{0}]" -f $item.Properties.displayName), "Remove")) {
                $itemDeleted = Remove-AzPolicySetDefinition -Id $item.ResourceId -Force -Confirm:$false -WarningAction SilentlyContinue
                $itemDeleted
                if ($itemDeleted) {
