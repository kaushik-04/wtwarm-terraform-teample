<#
	...
	.DESCRIPTION
		This script contains functionality used to test ARM template syntax.
...
$Parameters = @()
$ParameterFiles = (Get-ChildItem (Join-Path -Path $($(Get-Item $PSScriptRoot).Parent.FullName) -ChildPath $ParameterFilePath -AdditionalChildPath "machinelearning*.parameters.json") -Recurse).Name
...
Describe "Template: $(Split-Path $($(Get-Item $PSScriptRoot).Parent.FullName) -Leaf)" -Tags Unit {
	...
	Context "Parameter File Syntax" {
		...
		It "Parameter file (<ParameterFileName>)  does contain all expected properties" -TestCases $Parameters {
	...
	Context "Template and Parameter Compatibility" {
		...
		It "Count of required parameters in template file ($TemplateFileName) is equal or less than count of all parameters in parameters file (<ParameterFileName>)" -TestCases $Parameters {
			Param ($ParameterFileName)
		...
		It "All parameters in parameters file (<ParameterFileName>) exist in template file ($TemplateFileName)" -TestCases $Parameters {
			Param ($ParameterFileName)
		...
		It "All required parameters in template file ($TemplateFileName) existing in parameters file (<ParameterFileName>)" -TestCases $Parameters {
			Param ($ParameterFileName)
