            ...
            if ($AssignmentInJson.managedIdentity.assignIdentity -eq $true -and $AssignmentInJson.managedIdentity.location) {
                ...
                if ($roleDefinitionIds.Count -gt 0) {
                    foreach ($roleDefinitionId in $roleDefinitionIds) {
                        ...
                        do {
                            Write-Output "        Waiting for policy assignments Managed Identity to propagate to Azure AD"
                            Start-Sleep -Seconds $RetryInterval
                            $ID = Get-AzADServicePrincipal -ObjectId $NewAssignment.Identity.PrincipalId -ErrorAction SilentlyContinue
                        } while ($null -eq $ID)
                        Write-Output "        Found policy assignments Managed Identity in Azure AD - $($ID.id)"
