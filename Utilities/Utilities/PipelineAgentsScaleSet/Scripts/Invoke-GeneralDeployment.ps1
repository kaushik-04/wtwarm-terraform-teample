...
.EXAMPLE
Invoke-GeneralDeployment -resourcegroupName 'WVD-HostPool-01-PO-RG' -modulePackagePath '/azp/agent/_work/3/s/ModulePackages/WVDHostPool' -parameterFilePath $parameterFilePath

Deploy a host pool into the resource group 'WVD-HostPool-01-PO-RG' using the previously downloaded modile files in path '/azp/agent/_work/3/s/ModulePackages/WVDHostPool'

.EXAMPLE
Invoke-GeneralDeployment -resourcegroupName 'WVD-HostPool-01-PO-RG' -modulePackagePath '/azp/agent/_work/3/s/ModulePackages/WVDHostPool' -parameterFilePath $parameterFilePath -location 'WestEurope'

Deploy a host pool into the resource group 'WVD-HostPool-01-PO-RG' using the previously downloaded modile files in path '/azp/agent/_work/3/s/ModulePackages/WVDHostPool' using the explicit location 'WestEurope'
	...
	process {
		...
		if ($Deployment.Outputs) {
			foreach ($Outputkey in $Deployment.Outputs.Keys) {
				Write-Verbose ("Set [{0}] deployment output as pipeline environment variable with value: [{1}]" -f $Outputkey, $Deployment.Outputs[$Outputkey].Value) -Verbose
