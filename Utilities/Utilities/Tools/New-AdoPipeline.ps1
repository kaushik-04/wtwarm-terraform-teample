...
Optional. Choose which Modules to create pipelines for. By default, Pipelines will be created for all modules in the PipelineSourcePath .
...
function New-AdoPipeline {
  ...
  param (
    ...
    [Parameter(Mandatory = $false, HelpMessage = "Azure DevOps branch: <BranchName>")][string]$BranchName = "master",
    [Parameter(Mandatory = $false, HelpMessage = "Example: Modules")][string]$PipelineTargetPath = "Modules",
    [Parameter(Mandatory = $false, HelpMessage = "Example: C:\Modules\")][string]$PipelineSourcePath = "/Modules",
    [Parameter(Mandatory = $false, HelpMessage = "Default: `$false")][bool]$CreateBuildValidation = $false,
    [Parameter(Mandatory = $false, HelpMessage = "Example: @(`"ActivityLog`",`"ActivityLogAlert`")")][array]$ModuleFilter
  )
  ...
  try {
    ...
    foreach ($pipeline in $ymlPipelines) {
      ...
      $moduleName = $pathSplit[-3] # here we have the module name
    ...
    if ($ModuleFilter){
      Write-Verbose "Filtering pipelines based on ModuleFilters parameter"
      $pipelinesToBeUpdated = $pipelinesToBeUpdated | Where-Object {$_.moduleName -in $ModuleFilter }
    }
