...
param (
    ...
    [Parameter(Mandatory = $false,
        HelpMessage = "When using this switch, the script deletes all Initiative definitions from Azure that are not included somewhere within the policy definition root folder.")]
...
foreach ($policySet in $policySets) {
    ...
    $policySetObj = $Json | ConvertFrom-Json -Depth 100
...
foreach ($policySetObject in $policySetObjects) {
    ...
    $policySetDefinitionConfig = @{
        ...
        PolicyDefinition = $policySetObject.policyDefinitions | ConvertTo-Json -Depth 100 -AsArray
    ...
    #Add policySet groupDefinitions if present in the definition file       
    if($policySetObject.policyDefinitionGroups) {
        $policySetDefinitionConfig.Add("GroupDefinition", ($policySetObject.policyDefinitionGroups | ConvertTo-Json -Depth 100 -AsArray))
    }
    ...
    # Update scenario
        ...
        foreach ($policyDefinition in $policySetObj.policyDefinitions) {
        ...
        foreach ($existingDefinition in $matchingCustomDefinitionInAzure.Properties.policyDefinitions) {
            if (-not ($policySetObject.policyDefinitions.policyDefinitionId).Contains($existingDefinition.policyDefinitionId)) {
                Write-Verbose  "        The policyDefinitionId" -Verbose
                Write-Verbose  "            ""$($existingDefinition.policyDefinitionId)""" -Verbose
...
#region Delete policySet definitions

# Delete policySet definitions that are not defined in any files within the policySet definition root folder
if ($definitionSetsToDelete.count -gt 0) {
    if ($DeleteIfMarkedForDeletion.IsPresent) {
        Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
        Write-Verbose  "Deleting Initiative definitions" -Verbose
        Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose

        foreach ($item in $definitionSetsToDelete) {
            Write-Warning "Deleting Initiative definition: $($item.Properties.displayName) - $($item.ResourceId)"

            if ($PSCmdlet.ShouldProcess(("PolicySet definition [{0}]" -f $item.Properties.displayName), "Remove")) {
                $itemDeleted = Remove-AzPolicySetDefinition -Id $item.ResourceId -Force -Confirm:$false -WarningAction SilentlyContinue -ErrorAction SilentlyContinue
                $itemDeleted
                if ($itemDeleted) {
                    $noOfPolicySetsDeleted++
                    Write-Verbose  "" -Verbose
                    Write-Verbose  "        Initiative definition successfully deleted from Azure." -Verbose
                    Write-Verbose  "_____________________________________________________________" -Verbose
                }
                else {
                    Write-Error "Deleting the initiative definition $($item.Properties.displayName) failed. Check if this initiative is still assigned on any scope." -ErrorAction Stop
                }
                Write-Verbose  "" -Verbose
                $itemDeleted = $null
            }
        ...
        Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
        Write-Verbose  "Initiative marked for deletion" -Verbose
        Write-Verbose  "-------------------------------------------------------------------------------------------------------------------" -Verbose
        Write-Warning "    The following Initiative definitions are not defined in any JSON files within the Initiative definitions folder provided. These should be deleted by running this script, using the ""-DeleteIfMarkedForDeletion"" switch."
        foreach ($item in $definitionSetsToDelete) {
            Write-Verbose  "    $($item.Properties.displayName) - $($item.ResourceId)" -Verbose
        }
        Write-Verbose  "" -Verbose
