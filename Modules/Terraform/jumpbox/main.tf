...
resource "azurerm_key_vault_secret" "this" {
  for_each     = var.linux_vms
  name         = format("%s00%d-secret", each.value.name, index(keys(var.linux_vms), each.key) + 1)
...
resource "azurerm_linux_virtual_machine" "linux_vms" {
  ...
  os_disk {
    ...
    disk_encryption_set_id    = coalesce(lookup(each.value, "enable_cmk_disk_encryption"), false) == true && ((local.keyvault_state_exists == true ? data.terraform_remote_state.keyvault.outputs.purge_protection : data.azurerm_key_vault.this.0.purge_protection_enabled) == true) ? lookup(azurerm_disk_encryption_set.this, each.key)["id"] : null
  }
...
locals {
  linux_nics_with_app_gateway_list = flatten([
    for vm_k, vm_v in var.linux_vms : [
      for backend_pool in lookup(data.terraform_remote_state.applicationgateway.outputs.application_gateway_backend_pools, vm_v.app_gateway_name, []) :
      {
        key                     = "${vm_k}_${backend_pool}"
        nic_key                 = vm_v.vm_nic_keys[0]
        backend_address_pool_id = lookup(data.terraform_remote_state.applicationgateway.outputs.application_gateway_backend_pool_ids_map, backend_pool, null)
      }
