...
resource "azurerm_monitor_scheduled_query_rules_alert" "this" {
  ...
  authorized_resource_ids = local.loganalytics_state_exists == true ? list(lookup(data.terraform_remote_state.loganalytics.outputs.law_id_map, each.value["law_name"])) : list(lookup(data.azurerm_log_analytics_workspace.this, each.key)["id"])
  description             = lookup(each.value, "description", null)
