...
locals {
  tags                       = merge(var.app_gateway_additional_tags, (local.resourcegroup_state_exists == true ? lookup(data.terraform_remote_state.resourcegroup.outputs.resource_group_tags_map, var.resource_group_name) : data.azurerm_resource_group.this.0.tags))
...
data "azurerm_subnet" "frontend_ip_subnet" {
  for_each             = local.networking_state_exists == false ? { for x in local.frontend_ip_subnet_names : x.key => x } : {}
...
data "azurerm_subnet" "gateway_ip_subnet" {
  for_each             = local.networking_state_exists == false ? { for x in local.gateway_ip_subnet_names : x.key => x } : {}
  name                 = each.value.subnet_name
  virtual_network_name = each.value.vnet_name
  resource_group_name  = each.value.networking_resource_group != null ? each.value.networking_resource_group : (local.resourcegroup_state_exists == true ? var.resource_group_name : data.azurerm_resource_group.this.0.name)
}
...
resource "azurerm_application_gateway" "this" {
  ...
  dynamic "gateway_ip_configuration" {
    ...
    content {
      ...
      subnet_id = local.networking_state_exists == true ? lookup(data.terraform_remote_state.networking.outputs.map_subnet_ids, gateway_ip_configuration.value.subnet_name, null) : lookup(data.azurerm_subnet.gateway_ip_subnet, "${each.key}_${gateway_ip_configuration.value.name}")["id"]
    }
  }
  ...
  dynamic "frontend_ip_configuration" {
    ...
    content {
      ...
      subnet_id                     = coalesce(lookup(frontend_ip_configuration.value, "enable_public_ip"), false) == true ? null : (local.networking_state_exists == true ? lookup(data.terraform_remote_state.networking.outputs.map_subnet_ids, frontend_ip_configuration.value.subnet_name, null) : lookup(data.azurerm_subnet.frontend_ip_subnet, "${each.key}_${frontend_ip_configuration.value.name}")["id"])
